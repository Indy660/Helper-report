...
async function createAIReport(reportData, reportDate) {
    ...
    const completion = await client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "Ты помощник-аналитик, составляешь структурированные еженедельные отчёты для команды разработки."
            },
            {
                role: "user",
                content: `Составь единый общий отчет за неделю для документа Word строго в следующем формате:

1. Заголовок: "Общий отчет за неделю" (без выделений, без Markdown, просто текст).  
2. Вводное предложение на 1–2 предложения о проделанной работе.  
3. Раздел "Основные выполненные задачи" (без выделений). Далее перечисление:  
   • пункты первого уровня  
     o подпункты второго уровня (если есть, начинай их с таба для смещения вправо относительно основного пункта).  
   Не указывай имена сотрудников, только выполненные задачи (без их номеров).  
4. Раздел "Внедрение новых и сопровождение текущих сотрудников" (если такие задачи встречаются).  
5. Раздел "Глобальное обновление проектов" (если такие задачи встречаются, иначе не нужно).  
6. Раздел "Изучение новых технологий" (если такие задачи встречаются, иначе не нужно).  
7. Раздел "Выводы". Внутри 2–8 тезисов в формате ✅.  
8. В самом конце всегда добавляй строку:  
   "План на ближайшее время: улучшение кодовой базы, исправление ошибок и оптимизация работы приложений."

Формат вывода — только чистый текст, без Markdown, без звёздочек, без решёток.  
Также требуется отступы между разделами.
...
(async () => {
    try {
        const { result, reportDate } = await getWeeklyReport(); // можно null для текущей недели
